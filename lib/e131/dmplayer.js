...
function DMPLayer(arg) {
  if (this instanceof DMPLayer === false) {
    return new DMPLayer(arg);
  }
  var buffer;
  ...
  if (Buffer.isBuffer(arg)) {
    ...
    buffer = Buffer.alloc(11 + arg, 0x00);
    buffer.writeUInt16BE(0x720b, 0); // flength
    buffer.writeUInt8(DMPLayer.VECTOR, 2); // vector
    buffer.writeUInt8(0xa1, 3); // type
    buffer.writeUInt16BE(0x0000, 4); // firstAddress
    buffer.writeUInt16BE(0x0001, 6); // addressIncrement
    buffer.writeUInt16BE(arg + 1, 8); // propertyValueCount
  }
